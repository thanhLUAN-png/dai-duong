import { useState } from 'react'
import { Link, useNavigate } from 'react-router'
import { useApp } from '../context/AppContext'

export default function Register() {
  const { register } = useApp()
  const navigate = useNavigate()
  const [form, setForm] = useState({ username: '', displayName: '', password: '', confirm: '' })
  const [error, setError] = useState('')
  const [loading, setLoading] = useState(false)

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault()
    if (form.password !== form.confirm) {
      setError('Mật khẩu xác nhận không khớp')
      return
    }
    if (form.password.length < 6) {
      setError('Mật khẩu phải có ít nhất 6 ký tự')
      return
    }
    setLoading(true)
    setTimeout(() => {
      const result = register(form.username, form.displayName, form.password)
      if (result.ok) {
        navigate('/Studio')
      } else {
        setError(result.error ?? 'Đăng ký thất bại')
      }
      setLoading(false)
    }, 500)
  }

  const fields = [
    { key: 'username', label: 'Tên tài khoản', placeholder: 'vd: nemo2026', type: 'text' },
    { key: 'displayName', label: 'Tên hiển thị', placeholder: 'vd: Ngư Dân Tí Hon', type: 'text' },
    { key: 'password', label: 'Mật khẩu', placeholder: '••••••••', type: 'password' },
    { key: 'confirm', label: 'Xác nhận mật khẩu', placeholder: '••••••••', type: 'password' },
  ] as const

  return (
    <div className="min-h-[calc(100vh-4rem)] flex items-center justify-center px-4 py-16">
      <div className="w-full max-w-md">
        <div
          className="rounded-3xl p-8 md:p-10"
          style={{
            background: 'rgba(255,255,255,0.85)',
            backdropFilter: 'blur(20px)',
            boxShadow: '0 20px 60px rgba(11,37,69,0.12)',
            border: '1.5px solid rgba(20,136,198,0.15)',
          }}
        >
          <div className="text-center mb-8">
            <div className="text-5xl mb-3 float-anim inline-block">🪼</div>
            <h1 className="text-3xl font-black" style={{ color: '#0b2545' }}>
              Tham gia cùng chúng mình!
            </h1>
            <p className="text-sm font-semibold mt-2" style={{ color: '#4a7fa5' }}>
              Tạo tài khoản để vẽ và thả cá vào đại dương chung
            </p>
          </div>

          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            {fields.map(({ key, label, placeholder, type }) => (
              <div key={key}>
                <label className="block text-sm font-black mb-1.5" style={{ color: '#0b2545' }}>
                  {label}
                </label>
                <input
                  type={type}
                  value={form[key]}
                  onChange={(e) => setForm({ ...form, [key]: e.target.value })}
                  placeholder={placeholder}
                  required
                  className="w-full px-4 py-3 rounded-xl text-sm font-semibold outline-none transition-all"
                  style={{
                    background: '#f0f9ff',
                    border: '2px solid #b8e4f9',
                    color: '#0b2545',
                  }}
                  onFocus={(e) => (e.target.style.borderColor = '#1488c6')}
                  onBlur={(e) => (e.target.style.borderColor = '#b8e4f9')}
                />
              </div>
            ))}

            {error && (
              <div
                className="px-4 py-3 rounded-xl text-sm font-bold"
                style={{ background: '#fff0f0', color: '#cc3333', border: '1.5px solid #ffcccc' }}
              >
                ⚠️ {error}
              </div>
            )}

            <button
              type="submit"
              disabled={loading}
              className="w-full py-3.5 rounded-xl font-black text-white text-base transition-all hover:scale-[1.02]"
              style={{
                background: loading ? '#90c0dc' : 'linear-gradient(135deg, #ff6b6b, #ff4444)',
                boxShadow: '0 6px 20px rgba(255,107,107,0.35)',
              }}
            >
              {loading ? '🌊 Đang tạo tài khoản...' : '🐠 Tạo tài khoản & Vào Xưởng vẽ'}
            </button>
          </form>

          <p className="text-center text-sm font-semibold mt-6" style={{ color: '#4a7fa5' }}>
            Đã có tài khoản?{' '}
            <Link
              to="/Account/Login"
              className="font-black"
              style={{ color: '#1488c6', textDecoration: 'none' }}
            >
              Đăng nhập
            </Link>
          </p>
        </div>
      </div>
    </div>
  )
}
